      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          {executives.map((member, index) => (
            <div 
              key={index}
              className="flex flex-col lg:flex-row gap-6 bg-gray-50 dark:bg-gray-800 rounded-2xl p-6 shadow-md"
            >
              <div className="w-32 h-32 mx-auto lg:mx-0 flex-shrink-0 overflow-hidden rounded-full border-4 border-blue-100 dark:border-blue-900">
                <img 
                  src={member.image} 
                  alt={member.name}
              ...
              <div>
                <h3 className="text-2xl font-semibold text-gray-900 dark:text-white mt-4 lg:mt-0">{member.name}</h3>
                <p className="text-blue-600 dark:text-blue-400 font-medium text-lg mb-3">{member.position}</p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {teamMembers.map((member, index) => (
            <div 
              key={index} 
              className="bg-gray-50 dark:bg-gray-800 rounded-2xl shadow-md p-6 text-center transition-transform duration-300 hover:-translate-y-2"
            >
